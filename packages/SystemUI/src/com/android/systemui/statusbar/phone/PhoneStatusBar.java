...
/** GANBAROU_PATCH_START **/
		...
		if (DEBUG) Slog.d("TopNavBar", "get current value mStatusBarNav = " + mStatusBarNav);
/** GANBAROU_PATCH_END **/
        }
    }
...
/** Check if we are in Expanded Desktop Mode and enable Navigation buttons if we area
	...
	but if we are not a tablet, just ignore this! **/
		if (mNavigationBarView != null) { /** Does the device show a navigation bar at all? **/
			if (DEBUG) Slog.d("TopNavBar", "mNavigationBarView != null" + mNavigationBarView);
			mTop_recent = mStatusBarWindow.findViewById(R.id.top_recent);
			mTop_home = mStatusBarWindow.findViewById(R.id.top_home);
			mTop_back = (ImageView) mStatusBarWindow.findViewById(R.id.top_back);
			mTop_menu = mStatusBarWindow.findViewById(R.id.top_menu);
			if (mStatusBarNav == 0) { /** Is the navigation in the status bar enabled? **/
				if (DEBUG) Slog.d("TopNavBar", "mStatusBarNav == 0" + mStatusBarNav);
				if (DEBUG) Slog.d("TopNavBar", "getExpandedDesktopMode = " + (getExpandedDesktopMode()));
				if (getExpandedDesktopMode() == 1) {
					if (DEBUG) Slog.d("TopNavBar", "Switch nav buttons on (makeStatusBarView)");
				...
				if (DEBUG) Slog.d("TopNavBar", "Switch nav buttons off (makeStatusBarView)");
				mTop_recent.setVisibility(View.GONE);
				mTop_home.setVisibility(View.GONE);
				mTop_back.setVisibility(View.GONE);
...
/** GANBAROU_PATCH_START **/
		if (mNavigationBarView != null) {
			if (mStatusBarNav == 0) { /** Is the navigation in the status bar enabled? **/
				if (getExpandedDesktopMode() == 1) { /** Is the expanded desktop enabled? **/
					mTop_back = (ImageView) mStatusBarWindow.findViewById(R.id.top_back);
					mTop_back.setImageResource(altBack ? R.drawable.gan_ic_sysbar_back_ime : R.drawable.gan_ic_sysbar_back);
				}
			}
		}
/** GANBAROU_PATCH_END **/
